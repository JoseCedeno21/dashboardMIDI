...
and exposed as \`req.me\`.)`
  ...
  fn: async function (inputs, exits) {
    ...
    //var myquery = "SELECT  FROM usuarios";
    //Usuarios.getDatastore().sendNativeQuery(myquery, function(err, usuario){
    //  sails.log(err);
    //  sails.log(usuario)
   //   sails.log(usuario.rows[0]);
    //})
    ...
    var userRecord = await Usuarios.findOne({
      correo: inputs.emailAddress.toLowerCase(),
    });
    //sails.log(userRecord.id );
    // Look up by the email address.
    // (note that we lowercase it to ensure the lookup is always case-insensitive,
    // regardless of which database we're using)
    var userRecords = await User.findOne({
